...
// Mismo .h que el hecho en clase y para la tarea
template <class T>
class BST {
private:
    TNode<T>* root;
...
public:
    ...
    bool remove(T data);
    int height();
    int heightAux(TNode<T>* node); //  Para ayudar a height
    // Empieza seccion de visit
    void visit(int type);
    void preorder(TNode<T>* node);
    void inorder(TNode<T>* node);
    void postorder(TNode<T>* node);
    void levelByLevel(); // Ultima funcion de visit
    void ancestors(T data);
    int whatLevelAmI(T data);
    int findLevel(TNode<T>* node, T data, int level);
    int countIpReps(T data); // Para contar las repeticiones de una misma ip
};
...
template <class T>
int BST<T>::height(){
    // Mi version pasada no contemplaba un caso de prueba asi que la corregi
    return height(root);
}

template <class T>
int BST<T>::heightAux(TNode<T>* node){
    // Esta funcion es para llamar a height recursivamente
    if (node == nullptr){
        return 0;
    } 
    // Llamamos a la funcion recursiva para el lado izquierdo y derecho del nodo
    int lHeight = heightAux(node->left);
    int rHeight = heightAux(node->right);
    // Finalmente regresamos el nivel de cada nodo
...
// Funcion para contar las repeticiones de las ips 

// Implementé esta función para la opción 4; la función ya supone que el dato existe y que los datos del árbol están en orden consecutivo
template<class T>
int BST<T> :: countIpReps(T data) {
    // Esta funcion asume que la Ip existe y que los datos estan ya ordenados
    TNode<T>* aux = root; // Apuntador auxiliar apuntado a la raiz
    int cont = 0; // Empezamos la cuenta en 0
    // Mientras el apuntador encuentra una direccion no nula seguimos buscando iteraciones de la ip
    while(aux != nullptr) {
        if(aux->data > data){ 
            aux = aux->left;
        } else if(aux->data < data) {
            aux = aux->right;
        } else { // Si encontramos el dato lo sumamos y recorremos aux a la derecha
            cont++;
            aux = aux->right;
        }
    }
    // Regresamos la cantidad total de veces que aparece la Ip en los registros
    return cont;
}
